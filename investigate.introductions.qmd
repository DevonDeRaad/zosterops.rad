...
#see details on the vcf
vcfR
#read in sample info csv
sample.info<-read.csv("~/Desktop/cali.zosterops.rad/zosterops.trimmed.RAD.sampling.csv")
#trim to retain only samples that passed filtering protocols and are contained in the vcf
sample.info<-sample.info[sample.info$ID %in% colnames(vcfR@gt),]
#check how many samples are retained per species
table(sample.info$Species)
#see sample order
colnames(vcfR@gt)
#make sure sample info DF order matches order of samples in vcf (this should return all TRUE)
